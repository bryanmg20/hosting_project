...
2. docker-compose up
3. En tu navegador digita la url: http://ui.localhost
...
#### 1.4 Tokens JWT con expiración
El sistema utiliza dos niveles de autenticación basados en tokens: los que provee Roble Auth y los JWT propios generados por el backend. Cuando un usuario se registra o inicia sesión, primero se autentica contra Roble Auth, que retorna un access token y un refresh token externos. Estos tokens se almacenan temporalmente en el backend para permitir realizar operaciones autorizadas sobre la base de datos Roble (lectura, escritura, inserción o eliminación de proyectos y usuarios). Sin embargo, para proteger las rutas internas de la API, el backend genera además sus propios JWT, que el cliente usa en cada solicitud. El JWT de acceso tiene una expiración corta de 15 minutos, lo que reduce la ventana de ataque en caso de robo del token; mientras que el refresh token tiene una duración de 7 días, permitiendo renovar la sesión sin volver a pedir credenciales. Este doble sistema asegura que las operaciones sobre Roble DB se hagan con credenciales válidas pero aisladas del cliente y las rutas internas de la API están protegidas usando tokens firmados por el backend. Si el access token expira, el frontend usa el refresh token para solicitar uno nuevo; si ambos tokens expiran, el usuario debe volver a autenticarse. Con esto se combina seguridad, control del ciclo de vida de tokens y separación entre autenticación externa (Roble Auth) e interna (API).
#### 1.5 Validación estricta de datos
Todas las entradas externas son validadas de forma rigurosa antes de ser procesadas o persistidas. Esto incluye validación de formatos (email conforme a RFC, URL de repositorio válida), restricciones de longitud y caracteres permitidos para nombres de proyecto, y comprobaciones semánticas (por ejemplo, que la URL apunte a un repositorio público o exista). En las operaciones que crean recursos se verifica unicidad (por ejemplo, nombre de proyecto único por usuario) y se realizan comprobaciones adicionales contra Roble DB antes de confirmar la operación. Los errores de validación devuelven respuestas claras y códigos HTTP apropiados (400/409/422) para que el frontend maneje correctamente cada caso.

### 2. Optimización de Recursos

#### 2.1 Apagado automático por inactividad
Para mejorar la eficiencia del servidor, los contenedores de los proyectos se apagan automáticamente luego de un periodo prolongado sin actividad. Esto evita que se mantengan procesos innecesarios consumiendo CPU y memoria, y permite reutilizar los recursos para nuevos despliegues. Cuando un contenedor recibe tráfico nuevamente, el sistema lo reinicia de forma transparente para el usuario. Este mecanismo optimiza el rendimiento general, reduce gastos computacionales y mantiene el servidor funcionando de manera ligera y estable.
#### 2.2 Límites de CPU y memoria por contenedor
Cada contenedor se crea con límites estrictos de uso de recursos, asignando un máximo de CPU y memoria permitidos. De este modo, se garantiza que ningún proyecto pueda consumir más recursos de los asignados y afectar la estabilidad del sistema o de otros usuarios. Esto mantiene un entorno justo y predecible, especialmente cuando múltiples proyectos se ejecutan simultáneamente. El aislamiento a nivel de recursos evita que errores de programación, ciclos infinitos o cargas excesivas deterioren el rendimiento global del servidor.
#### 2.3 Reutilización y limpieza de imágenes
La plataforma utiliza un sistema de gestión inteligente para las imágenes Docker asociadas a las plantillas de proyectos. Cada proyecto descarga las imágenes necesarias para su ejecución, pero si esas imágenes ya han sido descargadas previamente por otro proyecto, el sistema las reutiliza automáticamente sin necesidad de volver a descargarlas. Esto acelera enormemente los despliegues y reduce el consumo de ancho de banda y almacenamiento. Cuando un proyecto se elimina, el sistema detiene su contenedor, elimina el contenedor asociado y limpia las capas de imágenes que ya no son utilizadas por ningún otro proyecto. Este enfoque mantiene el entorno Docker ordenado, eficiente y con el menor uso posible de espacio.
#### 2.4 SSE en lugar de polling
El sistema utiliza Server‑Sent Events (SSE) para entregar métricas y cambios de estado de contenedores al frontend en tiempo real, evitando polling periódico. Con SSE el servidor mantiene una conexión ligera y unidireccional hacia el cliente y envía eventos cuando hay cambios (estado, CPU, memoria, requests, errores), lo que reduce drásticamente la carga de request/response en el backend y disminuye el uso de CPU. SSE también facilita reconexiones automáticas desde el cliente y permite enviar eventos diferenciados por tipo (por ejemplo event: metric, event: status). 
